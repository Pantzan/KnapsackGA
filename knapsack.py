...
class Knapsack:	
	...
	def __init__(self):		
		...
		self.population = 0
		import sys
		# increase max recursion if long long stack comes
		iMaxStackSize = 15000
	...
	def details(self, weights, profits, opt, C, population):
	...
	def run(self):
		...
		if self.opt in newparents:
			print ("found in {} generation" .format(self.iterated))
		else:
			self.iterated += 1
			print("create generation again. Generation number - {} -" .format(self.iterated))
...
k = Knapsack()
k.details(weights, profits, opt, C, population)
k.run()